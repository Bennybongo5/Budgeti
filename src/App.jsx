...
export default function App() {
  ...
  const histItems = useMemo(() => { const curMoStr = curY + "-" + String(curM).padStart(2, "0"); const moSet = new Set([...months, curMoStr]); const items = [...txs]; moSet.forEach(mo => { const [y, m] = mo.split("-").map(Number); const lastD = ld(y, m); recs.forEach(r => { if (r.dateDebut && mo < r.dateDebut) return; if (!r.frequence || r.frequence === "mois" || r.frequence === "paie") { const j = r.frequence === "paie" || r.jour === "paie" ? 1 : r.jour === "fin" ? lastD : Math.min(r.jour, lastD); items.push({ id: "rc-" + r.id + "-" + mo, type: "depense", desc: r.desc, amount: r.amount, cat: r.cat, date: ymd(y, m, j), source: "rec", recId: r.id }); } else { getRecOccurrencesInMonth(r, y, m).forEach(date => items.push({ id: "rc-" + r.id + "-" + date, type: "depense", desc: r.desc, amount: r.amount, cat: r.cat, date, source: "rec", recId: r.id })); } }); rrecs.forEach(r => { if (r.dateDebut && mo < r.dateDebut) return; if (!r.frequence || r.frequence === "mois" || r.frequence === "paie") { const j = r.frequence === "paie" ? 1 : Math.min(r.jour, lastD); items.push({ id: "rr-" + r.id + "-" + mo, type: "revenu", desc: r.desc, amount: r.amount, date: ymd(y, m, j), source: "rr", rrId: r.id }); } else { getRecOccurrencesInMonth(r, y, m).forEach(date => items.push({ id: "rr-" + r.id + "-" + date, type: "revenu", desc: r.desc, amount: r.amount, date, source: "rr", rrId: r.id })); } }); dettes.forEach(d => { (d.paiementsAuto || []).filter(pa => +pa.montant > 0 && (!pa.dateDebut || mo >= pa.dateDebut)).forEach(pa => { const j = pa.jour === "paie" ? 1 : pa.jour === "fin" ? lastD : Math.min(+pa.jour || 1, lastD); items.push({ id: "dp-" + pa.id + "-" + mo, type: "depense", desc: d.nom, amount: +pa.montant, date: ymd(y, m, j), source: "dette", detteId: d.id, paiId: pa.id, payType: "auto" }); }); }); projets.forEach(p => { (p.paiementsAuto || []).filter(pa => +pa.montant > 0 && (!pa.dateDebut || mo >= pa.dateDebut)).forEach(pa => { const j = pa.jour === "paie" ? 1 : pa.jour === "fin" ? lastD : Math.min(+pa.jour || 1, lastD); items.push({ id: "pp-" + pa.id + "-" + mo, type: "depense", desc: p.nom, amount: +pa.montant, date: ymd(y, m, j), source: "projet", icon: p.icon, projetId: p.id, paiId: pa.id, payType: "auto" }); }); }); }); dettes.forEach(d => d.paiements.forEach(p => items.push({ id: "dm-" + p.id, type: "depense", desc: d.nom, amount: p.montant, date: p.date, source: "dette", detteId: d.id, paiId: p.id, payType: "manuel" }))); projets.forEach(p => p.versements.forEach(v => items.push({ id: "pm-" + v.id, type: "depense", desc: p.nom, amount: v.montant, date: v.date, source: "projet", icon: p.icon, projetId: p.id, versId: v.id, payType: "manuel" }))); return items.sort((a, b) => b.date.localeCompare(a.date)); }, [txs, recs, rrecs, dettes, projets, months, curY, curM]);
  ...
  const periodes = useMemo(() => {
    ...
    return paies.map((deb, i) => {
      ...
      const rrP = rrecs.reduce((s, r) => { if (r.dateDebut && deb.slice(0,7) < r.dateDebut) return s; if (r.frequence === "paie") return s + r.amount; if (r.frequence === "semaine" || r.frequence === "2semaines") return s + r.amount; for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) return s + r.amount; } return s; }, 0);
      const ch = recs.reduce((s, r) => { if (r.dateDebut && deb.slice(0,7) < r.dateDebut) return s; if (r.frequence === "paie" || r.jour === "paie") return s + r.amount; if (r.frequence === "semaine" || r.frequence === "2semaines") return s + r.amount; for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) return s + r.amount; } return s; }, 0);
