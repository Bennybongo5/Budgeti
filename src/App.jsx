...
import { getPaies, getFinD, calcAutoInPeriod, calcManuelsInPeriod, calcProportionalMonth, getRecOccurrencesInMonth, getWeeklyOccurrencesInPeriod } from "./utils/calculs.js";
...
export default function App() {
  ...
  const [paiFrm, setPaiFrm] = useState({ montant: "", date: today(), frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "1", dateDebut: today().slice(0,7) });
  const [editPai, setEditPai] = useState(null);
  const [editPaiFrm, setEditPaiFrm] = useState({ montant: "", date: "", frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "", dateDebut: today().slice(0,7) });
  ...
  const [verFrm, setVerFrm] = useState({ montant: "", date: today(), frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "1", dateDebut: today().slice(0,7) });
  const [editVer, setEditVer] = useState(null);
  const [editVerFrm, setEditVerFrm] = useState({ montant: "", date: "", frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "", dateDebut: today().slice(0,7) });
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", darkMode ? "#0A0A0A" : "#FFFFFF");
  }, [darkMode]);
  ...
  const delRec = id => { updRecs(p => p.map(r => r.id === id ? { ...r, dateFin: today().slice(0,7) } : r)); setERecId(null); setERecMod(false); };
  const excludeRecOcc = (recId, mo) => updRecs(p => p.map(r => r.id === recId ? { ...r, exclusions: [...(r.exclusions||[]), mo] } : r));
  ...
  const delRr = id => { updRrecs(p => p.map(r => r.id === id ? { ...r, dateFin: today().slice(0,7) } : r)); setERrId(null); setERrMod(false); };
  const excludeRrOcc = (rrId, mo) => updRrecs(p => p.map(r => r.id === rrId ? { ...r, exclusions: [...(r.exclusions||[]), mo] } : r));
  ...
  const addPai = () => { if (!paiFrm.montant || isNaN(+paiFrm.montant)) return; if (paiType === "fixe") { updDettes(p => p.map(d => d.id === detSel ? { ...d, paiementsAuto: [...(d.paiementsAuto || []), { id: "pf-" + Date.now(), montant: +paiFrm.montant, frequence: paiFrm.frequence || "mois", jourSemaine: paiFrm.jourSemaine || "Lundi", dateRef: paiFrm.dateRef || "", jour: paiFrm.jour || "1", dateDebut: paiFrm.dateDebut || "" }] } : d)); } else { updDettes(p => p.map(d => d.id === detSel ? { ...d, paiements: [...d.paiements, { id: "p-" + Date.now(), montant: +paiFrm.montant, date: paiFrm.date }] } : d)); } setPaiFrm({ montant: "", date: today(), frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "1", dateDebut: today().slice(0,7) }); setPaiType(null); setShowAddPai(false); };
  const delPai = (did, pid) => updDettes(p => p.map(d => d.id === did ? { ...d, paiements: d.paiements.filter(x => x.id !== pid) } : d));
  const delPaiFixe = (did, pid) => updDettes(p => p.map(d => d.id === did ? { ...d, paiementsAuto: (d.paiementsAuto || []).map(x => x.id === pid ? { ...x, dateFin: today().slice(0,7) } : x) } : d));
  const excludeDettePayOcc = (detteId, paiId, mo) => updDettes(p => p.map(d => d.id === detteId ? { ...d, paiementsAuto: (d.paiementsAuto||[]).map(pa => pa.id === paiId ? { ...pa, exclusions: [...(pa.exclusions||[]), mo] } : pa) } : d));
  const saveEditPai = () => { if (!editPaiFrm.montant || isNaN(+editPaiFrm.montant)) return; if (editPai.type === "fixe") { updDettes(p => p.map(d => d.id === detSel ? { ...d, paiementsAuto: (d.paiementsAuto || []).map(x => x.id === editPai.id ? { ...x, montant: +editPaiFrm.montant, frequence: editPaiFrm.frequence || "mois", jourSemaine: editPaiFrm.jourSemaine || "Lundi", dateRef: editPaiFrm.dateRef || "", jour: editPaiFrm.jour, dateDebut: editPaiFrm.dateDebut || "" } : x) } : d)); } else { updDettes(p => p.map(d => d.id === detSel ? { ...d, paiements: d.paiements.map(x => x.id === editPai.id ? { ...x, montant: +editPaiFrm.montant, date: editPaiFrm.date } : x) } : d)); } setEditPai(null); };
  ...
  const addVer = () => { if (!verFrm.montant || isNaN(+verFrm.montant)) return; if (verType === "fixe") { updProjets(p => p.map(x => x.id === prjSel ? { ...x, paiementsAuto: [...(x.paiementsAuto || []), { id: "pf-" + Date.now(), montant: +verFrm.montant, frequence: verFrm.frequence || "mois", jourSemaine: verFrm.jourSemaine || "Lundi", dateRef: verFrm.dateRef || "", jour: verFrm.jour || "1", dateDebut: verFrm.dateDebut || "" }] } : x)); } else { updProjets(p => p.map(x => x.id === prjSel ? { ...x, versements: [...x.versements, { id: "v-" + Date.now(), montant: +verFrm.montant, date: verFrm.date }] } : x)); } setVerFrm({ montant: "", date: today(), frequence: "mois", jourSemaine: "Lundi", dateRef: "", jour: "1", dateDebut: today().slice(0,7) }); setVerType(null); setShowAddVer(false); };
  const delVer = (pid, vid) => updProjets(p => p.map(x => x.id === pid ? { ...x, versements: x.versements.filter(v => v.id !== vid) } : x));
  const delVerFixe = (pid, vid) => updProjets(p => p.map(x => x.id === pid ? { ...x, paiementsAuto: (x.paiementsAuto || []).map(v => v.id === vid ? { ...v, dateFin: today().slice(0,7) } : v) } : x));
  const excludeProjetPayOcc = (projetId, paiId, mo) => updProjets(p => p.map(x => x.id === projetId ? { ...x, paiementsAuto: (x.paiementsAuto||[]).map(pa => pa.id === paiId ? { ...pa, exclusions: [...(pa.exclusions||[]), mo] } : pa) } : x));
  const saveEditVer = () => { if (!editVerFrm.montant || isNaN(+editVerFrm.montant)) return; if (editVer.type === "fixe") { updProjets(p => p.map(x => x.id === prjSel ? { ...x, paiementsAuto: (x.paiementsAuto || []).map(v => v.id === editVer.id ? { ...v, montant: +editVerFrm.montant, frequence: editVerFrm.frequence || "mois", jourSemaine: editVerFrm.jourSemaine || "Lundi", dateRef: editVerFrm.dateRef || "", jour: editVerFrm.jour, dateDebut: editVerFrm.dateDebut || "" } : v) } : x)); } else { updProjets(p => p.map(x => x.id === prjSel ? { ...x, versements: x.versements.map(v => v.id === editVer.id ? { ...v, montant: +editVerFrm.montant, date: editVerFrm.date } : v) } : x)); } setEditVer(null); };
  ...
  const histItems = useMemo(() => { const curMoStr = curY + "-" + String(curM).padStart(2, "0"); const moSet = new Set([...months, curMoStr]); const items = [...txs]; moSet.forEach(mo => { const [y, m] = mo.split("-").map(Number); const lastD = ld(y, m); recs.forEach(r => { if (r.dateDebut && mo < r.dateDebut) return; if (r.dateFin && mo >= r.dateFin) return; if ((r.exclusions||[]).includes(mo)) return; if (!r.frequence || r.frequence === "mois" || r.frequence === "paie") { const j = r.frequence === "paie" || r.jour === "paie" ? 1 : r.jour === "fin" ? lastD : Math.min(r.jour, lastD); items.push({ id: "rc-" + r.id + "-" + mo, type: "depense", desc: r.desc, amount: r.amount, cat: r.cat, date: ymd(y, m, j), source: "rec", recId: r.id }); } else { getRecOccurrencesInMonth(r, y, m).forEach(date => items.push({ id: "rc-" + r.id + "-" + date, type: "depense", desc: r.desc, amount: r.amount, cat: r.cat, date, source: "rec", recId: r.id })); } }); rrecs.forEach(r => { if (r.dateDebut && mo < r.dateDebut) return; if (r.dateFin && mo >= r.dateFin) return; if ((r.exclusions||[]).includes(mo)) return; if (!r.frequence || r.frequence === "mois" || r.frequence === "paie") { const j = r.frequence === "paie" ? 1 : Math.min(r.jour, lastD); items.push({ id: "rr-" + r.id + "-" + mo, type: "revenu", desc: r.desc, amount: r.amount, date: ymd(y, m, j), source: "rr", rrId: r.id }); } else { getRecOccurrencesInMonth(r, y, m).forEach(date => items.push({ id: "rr-" + r.id + "-" + date, type: "revenu", desc: r.desc, amount: r.amount, date, source: "rr", rrId: r.id })); } }); dettes.forEach(d => { (d.paiementsAuto || []).filter(pa => +pa.montant > 0 && (!pa.dateDebut || mo >= pa.dateDebut) && (!pa.dateFin || mo < pa.dateFin) && !((pa.exclusions||[]).includes(mo))).forEach(pa => { if (pa.frequence === "semaine" || pa.frequence === "2semaines") { getWeeklyOccurrencesInPeriod(pa.frequence, pa.jourSemaine, pa.dateRef, ymd(y, m, 1), addD(ymd(y, m, lastD), 1)).forEach(date => { if (!(pa.exclusions||[]).includes(date.slice(0,7))) items.push({ id: "dp-" + pa.id + "-" + date, type: "depense", desc: d.nom, amount: +pa.montant, date, source: "dette", detteId: d.id, paiId: pa.id, payType: "auto" }); }); } else { const j = pa.jour === "paie" || pa.frequence === "paie" ? 1 : pa.jour === "fin" ? lastD : Math.min(+pa.jour || 1, lastD); items.push({ id: "dp-" + pa.id + "-" + mo, type: "depense", desc: d.nom, amount: +pa.montant, date: ymd(y, m, j), source: "dette", detteId: d.id, paiId: pa.id, payType: "auto" }); } }); }); projets.forEach(p => { (p.paiementsAuto || []).filter(pa => +pa.montant > 0 && (!pa.dateDebut || mo >= pa.dateDebut) && (!pa.dateFin || mo < pa.dateFin) && !((pa.exclusions||[]).includes(mo))).forEach(pa => { if (pa.frequence === "semaine" || pa.frequence === "2semaines") { getWeeklyOccurrencesInPeriod(pa.frequence, pa.jourSemaine, pa.dateRef, ymd(y, m, 1), addD(ymd(y, m, lastD), 1)).forEach(date => { if (!(pa.exclusions||[]).includes(date.slice(0,7))) items.push({ id: "pp-" + pa.id + "-" + date, type: "depense", desc: p.nom, amount: +pa.montant, date, source: "projet", icon: p.icon, projetId: p.id, paiId: pa.id, payType: "auto" }); }); } else { const j = pa.jour === "paie" || pa.frequence === "paie" ? 1 : pa.jour === "fin" ? lastD : Math.min(+pa.jour || 1, lastD); items.push({ id: "pp-" + pa.id + "-" + mo, type: "depense", desc: p.nom, amount: +pa.montant, date: ymd(y, m, j), source: "projet", icon: p.icon, projetId: p.id, paiId: pa.id, payType: "auto" }); } }); }); }); dettes.forEach(d => d.paiements.forEach(p => items.push({ id: "dm-" + p.id, type: "depense", desc: d.nom, amount: p.montant, date: p.date, source: "dette", detteId: d.id, paiId: p.id, payType: "manuel" }))); projets.forEach(p => p.versements.forEach(v => items.push({ id: "pm-" + v.id, type: "depense", desc: p.nom, amount: v.montant, date: v.date, source: "projet", icon: p.icon, projetId: p.id, versId: v.id, payType: "manuel" }))); return items.sort((a, b) => b.date.localeCompare(a.date)); }, [txs, recs, rrecs, dettes, projets, months, curY, curM]);
  ...
  const periodes = useMemo(() => {
    ...
    return paies.map((deb, i) => {
      ...
      const rrP = rrecs.reduce((s, r) => { if (r.dateDebut && debMo < r.dateDebut) return s; if (r.dateFin && debMo >= r.dateFin) return s; if (r.frequence === "paie") { if ((r.exclusions||[]).includes(debMo)) return s; return s + r.amount; } if (r.frequence === "semaine" || r.frequence === "2semaines") { const occ = getWeeklyOccurrencesInPeriod(r.frequence, r.jourSemaine, r.dateRef, deb, fin); return s + occ.filter(d => !(r.exclusions||[]).includes(d.slice(0,7))).length * r.amount; } for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) { if ((r.exclusions||[]).includes(ds.slice(0,7))) return s; return s + r.amount; } } return s; }, 0);
      const ch = recs.reduce((s, r) => { if (r.dateDebut && debMo < r.dateDebut) return s; if (r.dateFin && debMo >= r.dateFin) return s; if (r.frequence === "paie" || r.jour === "paie") { if ((r.exclusions||[]).includes(debMo)) return s; return s + r.amount; } if (r.frequence === "semaine" || r.frequence === "2semaines") { const occ = getWeeklyOccurrencesInPeriod(r.frequence, r.jourSemaine, r.dateRef, deb, fin); return s + occ.filter(d => !(r.exclusions||[]).includes(d.slice(0,7))).length * r.amount; } for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) { if ((r.exclusions||[]).includes(ds.slice(0,7))) return s; return s + r.amount; } } return s; }, 0);
      ...
      <div style={{ padding: "14px 14px 0" }}>
        ...
        {view === "analyse" && (
          <Analyse
            ...
            excludeRecOcc={excludeRecOcc} excludeRrOcc={excludeRrOcc}
            excludeDettePayOcc={excludeDettePayOcc} excludeProjetPayOcc={excludeProjetPayOcc}
            updDettes={updDettes} updProjets={updProjets}
            {...styles}
          />
        )}
        {view === "parametres" && (
