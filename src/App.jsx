...
export default function App() {
  ...
  const periodes = useMemo(() => {
    ...
    return paies.map((deb, i) => {
      ...
      const rrP = rrecs.reduce((s, r) => { if (r.dateDebut && deb.slice(0,7) < r.dateDebut) return s; if (r.dateFin && deb.slice(0,7) > r.dateFin) return s; if (r.frequence === "paie") return s + r.amount; if (r.frequence === "semaine" || r.frequence === "2semaines") return s + r.amount; for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) return s + r.amount; } return s; }, 0);
      const ch = recs.reduce((s, r) => { if (r.dateDebut && deb.slice(0,7) < r.dateDebut) return s; if (r.dateFin && deb.slice(0,7) > r.dateFin) return s; if (r.frequence === "paie" || r.jour === "paie") return s + r.amount; if (r.frequence === "semaine" || r.frequence === "2semaines") return s + r.amount; for (let mo = 0; mo <= 1; mo++) { let mc = dm - 1 + mo, yc = dy + Math.floor(mc / 12); mc = ((mc % 12) + 12) % 12; const lastD = ld(yc, mc + 1); const j = r.jour === "fin" ? lastD : Math.min(+r.jour || 1, lastD); const ds = ymd(yc, mc + 1, j); if (ds >= deb && (fin ? ds < fin : true)) return s + r.amount; } return s; }, 0);
